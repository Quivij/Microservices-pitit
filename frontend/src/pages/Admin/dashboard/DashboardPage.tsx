  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let prevRevenueRes = [];
        try {
            prevRevenueRes = await getRevenueStats(prevFromStr, prevToStr, "month");
        } catch (e) {
            console.log("Error fetching previous revenue", e);
        }
        ...
        const prevTotalRevenue = prevRevenueRes.reduce((sum: any, d: any) => sum + d.revenue, 0);
        
        let growthPercent = 0;
        if (prevTotalRevenue === 0) {
           growthPercent = totalRevenue > 0 ? 100 : 0;
        } else {
           growthPercent = ((totalRevenue - prevTotalRevenue) / prevTotalRevenue) * 100;
        }
        const growthStr = growthPercent > 0 ? `+${growthPercent.toFixed(1)}%` : `${growthPercent.toFixed(1)}%`;
