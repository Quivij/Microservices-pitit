...
async function test() {
    ...
    const user = await db.collection('users').findOne({ email: 'vip1@gmail.com' });
    if (!user) { console.log('User not found'); process.exit(1); }
    
    const cart = await db.collection('carts').findOne({ user: user._id });
    if (!cart) { console.log('Cart not found'); process.exit(1); }
    
    // Add product to cart if empty
    if (cart.items.length === 0) {
        const product = await db.collection('products').findOne({});
        await db.collection('carts').updateOne({ _id: cart._id }, { $push: { items: { product: product._id, quantity: 1, isCommented: false } } });
        cart.items = [{ product: product._id, quantity: 1, isCommented: false }];
    }
    ...
    try {
        const res = await axios.post('http://localhost:6974/v1/api/payment/create-qr', payload, {
    ...
    if (fs.existsSync('./createqr_error.txt')) {
        console.log("CREATE_QR_ERROR:\n" + fs.readFileSync('./createqr_error.txt', 'utf8'));
    }
    process.exit(0);
}
test();
