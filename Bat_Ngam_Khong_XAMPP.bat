...
echo.
echo 2. Dang bat he thong Microservices (Frontend + Backend) ngam...
...
powershell -Command "$content = Get-Content 'cloudflare_log.txt' -ErrorAction SilentlyContinue | Out-String; if ($content -match '(https://[a-zA-Z0-9-]+\.trycloudflare\.com)') { $url = $matches[1]; node update_worker.js \"$url\"; '===================================================' | Out-File -Encoding ASCII 'Link_Truy_Cap.txt'; ' LINK TRUY CAP VINH VIEN (GUI CHO KHACH HANG):' | Out-File -Encoding ASCII -Append 'Link_Truy_Cap.txt'; ' https://vpp-stationery.estt.workers.dev/' | Out-File -Encoding ASCII -Append 'Link_Truy_Cap.txt'; '===================================================' | Out-File -Encoding ASCII -Append 'Link_Truy_Cap.txt'; '' | Out-File -Encoding ASCII -Append 'Link_Truy_Cap.txt'; 'Luu y: Chieu/Toi khi ve, ban nho CHAY FILE [Tat_Ngam_Khong_XAMPP.bat] nhe!' | Out-File -Encoding ASCII -Append 'Link_Truy_Cap.txt'; Start-Process notepad 'Link_Truy_Cap.txt'; Start-Process 'https://vpp-stationery.estt.workers.dev/'; } else { 'Mang hoi cham. Ban cho chut hoac mo file cloudflare_log.txt de xem nhe!' | Out-File -Encoding ASCII 'Link_Truy_Cap.txt'; Start-Process notepad 'Link_Truy_Cap.txt' }"
